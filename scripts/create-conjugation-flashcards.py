...
df = pd.read_csv(Path('outputs/conjugations-db.csv'),header=0)
...
with open('templates/conjugation-schema.json','r') as f:
    json_format = f.read()
...
df.to_csv(Path('outputs/conjugations-db.csv'), index=False)